...
public class MeshFactory {
    ...
    public static Mesh loadTerrain(int width, int height, int noiseMultiplier) throws IOException  {
        PerlinNoise pNoise = new PerlinNoise();
        float[][] noise = pNoise.GetNoise(width, height,0);
        // Criação dos vértices
        List<Vector3f> positions = new ArrayList<>();
        for (int z = 0; z < height; z++) {
            for (int x = 0; x < width; x++) {
                Random random = new Random();
                positions.add(new Vector3f(x, noise[x][z] * noiseMultiplier , z));
            }
        }
